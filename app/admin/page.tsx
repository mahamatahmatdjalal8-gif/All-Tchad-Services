  ...
  if (!user) {
    return (
      <main className="admin-denied">
        <div><span>AT</span><h1>Accès non autorisé</h1><p>Ce tableau de bord est réservé aux administrateurs d’Allô Tchad Services.</p><Link href="/">Retour au site</Link></div>
      </main>
    );
  }

  const db = getDb();
  const [requests, artisans, documents, feedback] = await Promise.all([
    db.select().from(serviceRequests).orderBy(desc(serviceRequests.createdAt)).limit(250),
    db.select().from(artisanApplications).orderBy(desc(artisanApplications.createdAt)).limit(250),
    db.select({
    ...
    }).from(artisanDocuments).orderBy(desc(artisanDocuments.createdAt)).limit(2500),
    db.select().from(feedbackEntries).orderBy(desc(feedbackEntries.createdAt)).limit(250),
  ]);
