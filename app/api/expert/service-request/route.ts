...
export async function POST(request: Request) {
  ...
  if (!context) return Response.json({ error: "Créez un compte ou connectez-vous pour envoyer une demande." }, { status: 401 });
  ...
  const db = tx;
  // Serialize submissions from this member so two clicks cannot create two missions.
  await db.select({ id: artisanApplications.id }).from(artisanApplications).where(eq(artisanApplications.id, context.expert.id)).for("update").limit(1);
  ...
  const [recent] = await db.select({ createdAt: serviceRequests.createdAt }).from(serviceRequests).where(and(eq(serviceRequests.requesterExpertId, context.expert.id), eq(serviceRequests.targetExpertId, target.id), eq(serviceRequests.requestKind, "service"))).orderBy(desc(serviceRequests.createdAt)).limit(1);
  ...
  notification = { requestId: mission.id, recipientType: "expert", recipientId: target.id, kind: "service_request", title: "Nouvelle demande de service", body: `${context.expert.name} sollicite votre service : ${details.slice(0, 120)}` };
  return Response.json({ requestId: mission.id, targetName: target.name, missionUrl: `/espace-expert?tab=missions&request=${mission.id}` }, { status: 201 });
  });
...
export async function PATCH(request: Request) {
  ...
  const result = await db.transaction(async (tx) => {
  const db = tx;
  const [mission] = await db.select().from(serviceRequests).where(and(eq(serviceRequests.id, requestId), eq(serviceRequests.requesterExpertId, context.expert.id))).for("update").limit(1);
  if (!mission || mission.requestKind !== "service") return Response.json({ error: "Cette demande ne vous appartient pas." }, { status: 403 });
  ...
  if (action === "accept_quote" || action === "reject_quote") {
    if (mission.expertDecision !== "accepted" || mission.quoteStatus !== "pending") return Response.json({ error: "Ce devis n’est plus en attente." }, { status: 409 });
    ...
    await db.update(serviceRequests).set({ status: "cancelled", cancellationReason: "Annulée par le client", updatedAt }).where(eq(serviceRequests.id, requestId));
    text = "La demande de service a été annulée par le client.";
  ...
  if (mission.targetExpertId) notification = { requestId, recipientType: "expert", recipientId: mission.targetExpertId, kind: action, title: action === "accept_quote" ? "Devis accepté" : action === "reject_quote" ? "Devis refusé" : "Demande annulée", body: text };
  const [updated] = await db.select().from(serviceRequests).where(eq(serviceRequests.id, requestId)).limit(1);
  return Response.json({ request: withoutAccessCode(updated), message });
  });
  if (notification && result.ok) await createNotification(notification).catch(() => console.error("Mission notification unavailable."));
  return result;
}
