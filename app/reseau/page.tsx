...
import { hasMissionConversation, ownedServiceMissions } from "../mission-access";
...
export default async function NetworkPage() {
  ...
  if (expertContext) {
    const owned = await db.select().from(serviceRequests).where(ownedServiceMissions(expertContext.expert.id)).orderBy(desc(serviceRequests.createdAt)).limit(200);
    const assignedRequests = owned.filter((item) => item.targetExpertId === expertContext.expert.id);
    const requestIds = new Set(owned.filter(hasMissionConversation).map((item) => item.id));
    const recentMessages = requestIds.size ? await db.select().from(requestMessages).where(and(inArray(requestMessages.requestId, [...requestIds]), ne(requestMessages.senderType, "system"), ne(requestMessages.senderExpertId, expertContext.expert.id))).orderBy(desc(requestMessages.createdAt)).limit(3) : [];
    ...
    const notifications = [
      ...socialNotices.map((notice) => ({ id: `notice-${notice.id}`, icon: (notice.kind.includes("comment") || notice.kind === "mission_message") ? "💬" : notice.kind.includes("like") ? "♥" : notice.kind.includes("follower") ? "👥" : "🔔", read: Boolean(notice.readAt), createdAt: notice.createdAt.toISOString(), title: notice.title, detail: notice.body.slice(0, 70), href: notice.requestId ? `/espace-expert?tab=${["mission_message", "mission_accepted"].includes(notice.kind) && requestIds.has(notice.requestId) ? "messages" : "missions"}&request=${notice.requestId}` : "/reseau" })),
