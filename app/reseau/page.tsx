...
export default async function NetworkPage() {
  ...
  if (expertContext) {
    ...
    const socialNotices = await db.select().from(notificationRows).where(and(eq(notificationRows.recipientType, "expert"), eq(notificationRows.recipientId, expertContext.expert.id), isNull(notificationRows.readAt))).orderBy(desc(notificationRows.createdAt)).limit(8);
    const notifications = [
      ...socialNotices.map((notice) => ({ id: `notice-${notice.id}`, icon: notice.kind.includes("comment") ? "💬" : notice.kind.includes("like") ? "♥" : notice.kind.includes("follower") ? "👥" : "🔔", title: notice.title, detail: notice.body.slice(0, 70), href: notice.requestId ? `/espace-expert?tab=missions&request=${notice.requestId}` : "/reseau" })),
