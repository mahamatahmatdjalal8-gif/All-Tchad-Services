  ...
  return <>
    <div className={`network-layout ${mode === "profile" ? "profile-mode" : ""}`}>
      ...
      <section className="feed-column">
        {filteredPosts.slice(0, visiblePostCount).map((post) => <article className="social-post" id={`publication-${post.id}`} key={post.id}>
          ...
          {post.requestId ? <div className="before-after-gallery"><figure><img src={`/api/social/images/${post.id}?kind=before`} alt={`Avant l’intervention de ${post.expert.name}`} /><figcaption>Avant</figcaption></figure><figure><img src={`/api/social/images/${post.id}?kind=after`} alt={`Après l’intervention de ${post.expert.name}`} /><figcaption>Après</figcaption></figure></div> : post.imageKey && <div className="post-image" role="img" aria-label={`Travail réalisé par ${post.expert.name}`} style={{ backgroundImage: `url(/api/social/images/${post.id})` }} />}
          ...
          <div className="post-actions"><button className={liked.includes(post.id) ? "active" : ""} onClick={() => toggleLike(post.id)}>♡ <span>J’aime</span></button><button onClick={() => { if (requireIdentity()) setCommentPost(commentPost === post.id ? null : post.id); }}>◯ <span>Commenter</span></button><button className={favoritePosts.includes(post.id) ? "active" : ""} onClick={() => togglePostFavorite(post.id)}>☆ <span>Enregistrer</span></button><button onClick={() => sharePost(post)}>↗ <span>{busy === `shared-${post.id}` ? "Lien copié" : "Partager"}</span></button><button onClick={() => reportPost(post.id)}>! <span>{busy === `reported-${post.id}` ? "Signalé" : "Signaler"}</span></button></div>
