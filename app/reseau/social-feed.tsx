...
import { Bookmark, ThumbsUp, MessageCircle, Share2, Flag } from "lucide-react";
    ...
    <div className={`network-layout ${mode === "home" ? "home-feed-only" : ""} ${mode === "profile" ? "profile-mode" : ""}`}>
      ...
      <section className="feed-column">
        {filteredPosts.slice(0, visiblePostCount).map((post) => <article className="social-post realization-card" id={`publication-${post.id}`} key={post.id}>
          <header><a className="expert-avatar" href={`/experts/${post.expert.id}`}>{post.expert.hasProfilePhoto ? <img src={`/api/expert/profile-photo?id=${post.expert.id}`} alt={`Photo de ${post.expert.name}`} /> : initials(post.expert.name)}</a><div><a className="post-expert-name" href={`/experts/${post.expert.id}`}>{post.expert.name}<i>✓</i></a><span>{post.expert.trade} · {post.expert.area}</span><small>{dateLabel(post.createdAt)}</small></div><details className="post-options"><summary aria-label="Options de la publication">⋯</summary><div className="post-options-panel">{currentExpertId !== post.expertId && <button className={followed.includes(post.expertId) ? "following" : ""} onClick={() => toggleRelation(post.expertId, "follow")}>{followed.includes(post.expertId) ? "Abonné ✓" : "+ Suivre"}</button>}<button className={favorites.includes(post.expertId) ? "saved" : ""} onClick={() => toggleRelation(post.expertId, "favorite")} aria-label="Enregistrer cet expert">{favorites.includes(post.expertId) ? "★" : "☆"}</button><button aria-pressed={favoritePosts.includes(post.id)} onClick={() => togglePostFavorite(post.id)}><Bookmark aria-hidden="true" />{favoritePosts.includes(post.id) ? "Enregistré" : "Enregistrer"}</button><button onClick={() => reportPost(post.id)}><Flag aria-hidden="true" />{busy === `reported-${post.id}` ? "Signalé" : "Signaler"}</button></div></details></header>
